...
## Journey from XOR Problem to Tensor Decomposition

**The Original Attempt**: The initial XOR Balanced Subsequences problem was generated by GitHub Copilot. It seemed promising with its position-based XOR parity condition, but when tested with Qwen3-235B-A22B-2507, the model solved it correctly on all test cases. The problem turned out to be mathematically equivalent to known XOR subsequence patterns that AI models have learned.

**The Pivot**: After the XOR problem failed to stump Qwen, I decided to leverage my background:

- **Tensor decomposition** was part of my bachelor term project at IIT
- I primarily work with **image processing** in my research
- I wanted to create something that genuinely integrates these domains

**The Insight**: Combining binary image matrices with tensor decomposition over Boolean gates creates a unique problem space that:

- Requires understanding from multiple fields (linear algebra, Boolean circuits, combinatorics)
- Isn't reducible to standard competitive programming patterns
- Has subtle mathematical properties that trip up pattern-matching approaches

This led to the Binary Image Tensor Gate Decomposition problem - a genuine fusion of image concepts, tensor algebra, and logic gate synthesis.
